use alloc::boxed::Box;
use environment::print::{get_debug_printer, set_debug_printer, Printer};
...
impl Printer for LoggedPrinter {
	fn print_bytes(&self, s: &[u8]) {
		...
		// Don't write into the kernel log buffer as it may call a printk
		// function due to an assertion.
...
macro_rules! debug_warn {
    ($fmt:expr) => {
        if cfg!(debug_assertions) {
            ::environment::println!(concat!("\x1b[1;33mWARN: ", $fmt, "\x1b[0m"));
        }
    };
    ($fmt:expr, $($arg:tt)*) => {
        if cfg!(debug_assertions) {
            ::environment::println!(concat!("\x1b[1;33mWARN: ", $fmt, "\x1b[0m"), $($arg)*);
        }
    };
}
