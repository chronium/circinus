...
static GLOBAL_QUEUE: SpinLock<SegQueue<Box<dyn JobCallback>>> =
	SpinLock::new(SegQueue::new());
...
impl DeferredJob {
	...
	pub fn run_later<F: JobCallback>(&self, callback: F) {
		GLOBAL_QUEUE.lock().push(box callback);
	}
}
...
pub fn run_deferred_jobs() {
	...
	//       interrupt handler. Should we have a limit of the maximum number of
	// jobs?
	//
