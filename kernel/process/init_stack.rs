...
use alloc::vec::Vec;
use api::{address::VAddr, Error, ErrorKind, Result};
...
fn push_aux_data_to_stack(
	...
	match auxv {
		Auxv::Random(values) => {
			push_bytes_to_stack(sp, stack_bottom, values.as_slice())?
		}
		_ => {}
	}
...
pub(super) fn estimate_user_init_stack_size(
	...
	let ptrs_len = (2 * (1 + auxv.len()) + argv.len() + 1 + envp.len() + 1 + 1)
		* size_of::<usize>();
