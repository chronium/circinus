...
impl VAddr {
	...
	pub const fn align_down(self, alignment: usize) -> Self {
		Self::new(align_down(self.0, alignment))
	}
...
///
/// Futhermore, like `NonNull<T>`, it is always non-null. Use
/// `Option<UserVaddr>` represent a nullable user pointer.
#[derive(Debug, Copy, Clone, Eq, PartialEq, Ord, PartialOrd, Hash)]
#[repr(transparent)]
...
impl UserVAddr {
	...
	#[inline(always)]
	pub const fn add(self, offset: usize) -> Self {
		unsafe { Self::new_unchecked(self.0 + offset) }
	}
	...
	/// Reads a string from the userspace and returns number of copied
