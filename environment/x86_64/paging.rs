...
use crate::{
	...
	page_allocator::{alloc_pages, AllocPageFlags, PageAllocError},
};
...
fn duplicate_table(
	...
	for i in 0..ENTRIES_PER_TABLE {
		...
		let new_paddr = if level == 1 {
			...
			if level == 4 && i >= 0x80 {
				// Kernel page table entries are immutable. Copy them as they
				// are.
				entry_paddr(entry)
			} else {
				// Create the deep copy of the referenced page table
				// recursively...
		...
		unsafe {
			*new_table.offset(i) =
				new_paddr.value() as u64 | entry_flags(entry);
		}
	}
...
fn allocate_pml4() -> Result<PAddr, PageAllocError> {
	...
	unsafe {
		let kernel_pml4 =
			PAddr::new(&__kernel_pml4 as *const u8 as usize).as_vaddr();
...
impl PageTable {
	...
	pub fn duplicate_from(
		original: &PageTable,
	) -> Result<PageTable, PageAllocError> {
