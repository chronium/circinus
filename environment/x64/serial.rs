...
impl SerialPort {
	pub const fn new(ioport_base: u16, irq: u8) -> Self {
		Self { ioport_base, irq }
	}
...
pub static SERIAL0: SerialPort = SerialPort::new(SERIAL0_IOPORT, SERIAL0_IRQ);
...
pub fn serial0_irq_handler() {
	while let Some(ch) = SERIAL0.receive_char() {
		if ch == b'\r' {
			system().on_console_rx(b'\n');
		} else {
			system().on_console_rx(ch);
		}
	}
}
