use crate::{
	address::{PAddr, VAddr},
	bootinfo::{BootInfo, RamArea, VirtioMmioDevice},
};
use arrayvec::{ArrayString, ArrayVec};
use core::{cmp::max, mem::size_of, slice};
use utils::{alignment::align_up, byte_size::ByteSize};
...
impl Cmdline {
	pub fn parse(cmdline: &[u8]) -> Cmdline {
		let s = core::str::from_utf8(cmdline)
			...
			for config in s.split(' ') {
				...
				match (words.next(), words.next()) {
					...
					(Some("virtio_mmio.device"), Some(value)) => {
						...
						info!(
							"bootinfo: virtio MMIO device: base={:016x}, \
							 irq={}",
							addr, irq
						);
...
unsafe fn parse_multiboot2_info(header: &Multiboot2InfoHeader) -> BootInfo {
	...
	while off + size_of::<Multiboot2TagHeader>() < header.total_size as usize {
		...
		match tag.tag_type {
			...
			6 => {
				// Memory map.
				let tag = &*(tag as *const Multiboot2TagHeader
					as *const Multiboot2MemoryMapTag);
	...
	BootInfo {
		...
		framebuffer: crate::bootinfo::Framebuffer {
			addr: PAddr::new(0),
			pitch: 0,
			width: 0,
			height: 0,
...
unsafe fn parse_multiboot_legacy_info(info: &MultibootLegacyInfo) -> BootInfo {
	...
	while off < info.memory_map_len {
		let entry: &MemoryMapEntry =
			&*PAddr::new((info.memory_map_addr + off) as usize).as_ptr();
		process_memory_map_entry(
			&mut ram_areas,
			entry.entry_type,
			entry.base as usize,
	...
	BootInfo {
		...
		use_second_serialport: cmdline.use_second_serialport,
		framebuffer: crate::bootinfo::Framebuffer {
			addr: PAddr::new(info.framebuffer_addr as usize),
