...
/// };
/// ```
fn get_bar_for_cfg_type(
	pci_device: &PciDevice,
	cfg_type: u8,
) -> Option<(&PciCapability, VAddr)> {
	let cap = pci_device.capabilities().iter().find(|cap| {
		cap.id == 9 && cap.data.len() >= 16 && cap.data[3] == cfg_type
	})?;

	let offset =
		u32::from_le_bytes(cap.data[8..12].try_into().unwrap()) as usize;
	match pci_device.config().bar(cap.data[4] as usize) {
		Bar::MemoryMapped { paddr } => {
			Some((cap, paddr.as_vaddr().add(offset)))
		}
		_ => {
...
impl VirtioPci {
	pub fn probe_pci<F, T>(
		pci_device: &PciDevice,
		ctor: F,
	) -> Result<T, VirtioAttachError>
	where
		F: FnOnce(Arc<dyn VirtioTransport>) -> Result<T, VirtioAttachError>,
	{
		let common_cfg =
			get_bar_for_cfg_type(pci_device, VIRTIO_PCI_CAP_COMMON_CFG)
				.expect("failed to locate pci_cap for common_cfg")
				.1;
		let device_cfg =
			get_bar_for_cfg_type(pci_device, VIRTIO_PCI_CAP_DEVICE_CFG)
		...
		let (notify, notify_off_multiplier) =
			get_bar_for_cfg_type(pci_device, VIRTIO_PCI_CAP_NOTIFY_CFG)
				.map(|(cap, mmio)| {
					...
					// queue_notify_off. */ };
					let notify_off_multiplier = u32::from_le_bytes(
						cap.data[16..20].try_into().unwrap(),
					);
...
impl VirtioTransport for VirtioPci {
	...
	fn read_device_config16(&self, offset: u16) -> u16 {
		unsafe { self.device_cfg.add(offset as usize).read_volatile::<u16>() }
	}

	fn read_device_config32(&self, offset: u16) -> u32 {
		unsafe { self.device_cfg.add(offset as usize).read_volatile::<u32>() }
	}
	...
	fn read_isr_status(&self) -> IsrStatus {
		IsrStatus::from_bits_truncate(unsafe {
			self.isr.add(0).read_volatile::<u8>()
		})
	}
