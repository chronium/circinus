...
impl VirtQueue {
	pub fn new(index: u16, transport: Arc<dyn VirtioTransport>) -> VirtQueue {
		...
		let avail_ring_size: usize =
			size_of::<u16>() * (3 + (num_descs as usize));
		let used_ring_off =
			align_up(avail_ring_off + avail_ring_size, PAGE_SIZE);
		let used_ring_size = size_of::<u16>() * 3
			+ size_of::<VirtqUsedElem>() * (num_descs as usize);
		...
		for i in 0..num_descs {
			let desc = unsafe {
				&mut *descs.as_mut_ptr::<VirtqDesc>().offset(i as isize)
			};
	...
	pub fn enqueue(&mut self, chain: &[VirtqDescBuffer]) {
		...
		if (self.num_free_descs as usize) < chain.len() {
			while self.last_used_index != self.used().index {
				let used_elem_index =
					self.used_elem(self.last_used_index).id as u16;
		...
		for (i, buffer) in chain.iter().enumerate() {
			...
			let (addr, len, flags) = match buffer {
				VirtqDescBuffer::ReadOnlyFromDevice { addr, len } => {
					(addr, *len, 0)
				}
	...
	pub fn can_pop(&self) -> bool {
		self.last_used_index != self.used().index
	}
}
...
impl Virtio {
	...
	pub fn initialize(
		...
		self.transport.write_device_status(
			self.transport.read_device_status() | VIRTIO_STATUS_ACK,
		);
		self.transport.write_device_status(
			self.transport.read_device_status() | VIRTIO_STATUS_DRIVER,
		);

		let device_features = self.transport.read_device_features();
		if (device_features & features) != features {
			warn!(
				"virtio: feature negotiation failure: driver={:x}, \
				 device={:x}, unspported={:x}",
				features,
				device_features,
		...
		self.transport.write_device_status(
			self.transport.read_device_status() | VIRTIO_STATUS_FEAT_OK,
		);
		...
		self.transport.write_device_status(
			self.transport.read_device_status() | VIRTIO_STATUS_DRIVER_OK,
		);
	...
	pub fn read_device_config16(&self, offset: u16) -> u16 {
		self.transport.read_device_config16(offset)
	}

	pub fn read_device_config32(&self, offset: u16) -> u32 {
		self.transport.read_device_config32(offset)
	}

	pub fn read_device_config64(&self, offset: u16) -> u64 {
		self.transport.read_device_config64(offset)
	}
