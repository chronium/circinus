...
use environment::address::PAddr;
use utils::alignment::is_aligned;
use x86::io::{inl, outl};

use super::VirtioKind;
...
impl PciDevice {
	...
	pub fn enable_bus_master(&self) {
		let bus = PciBus {};
		let value =
			bus.read32(self.bus, self.slot, pci_config_offset!(command));
		bus.write32(
			self.bus,
			self.slot,
	...
	pub fn is_virtio_kind(&self, kind: VirtioKind) -> bool {
		self.config.vendor_id == 0x1AF4 && self.config.device_id == kind as u16
	}
}
...
impl PciBus {
	...
	pub fn read_device_config(&self, bus: u8, slot: u8) -> Option<PciConfig> {
		...
		let header_type =
			self.read8(bus, slot, pci_config_offset!(header_type));
		if header_type != 0 {
			return None;
		}
	...
	fn next(&mut self) -> Option<Self::Item> {
		while !(self.bus_no == 255 && self.slot == 31) {
			...
			if let Some(config) = config {
				let capabilities =
					self.bus.read_capabilities(self.bus_no, self.slot - 1);
