...
impl CString {
	...
	/// obtained by calling [`into_raw`] on a `CString`. Other usage (e.g.
	/// trying to take ownership of a string that was allocated by foreign code)
	...
	/// Specifically, one should *not* use the standard C `free()` function to
...
impl CStr {
	...
	/// evaluated. To fix the problem, bind the `CString` to a local variable:
	///
	/// ```no_run
	/// # #![allow(unused_must_use)]
	/// use std::ffi::CString;
