...
for dep in `find $deps_dir -type f -name "*.rlib"`; do
    "${TARGET}-nm" --format=posix -g "$dep" 2>/dev/null | sed 's/.*:.*//g' | awk '{if ($2 == "T") print $1}' | sed 's/^\(.*\)$/\1 __cilibc_\1/g' >> $symbols_file
done

for special_sym in $special_syms; do
    echo "$special_sym __cilibc_$special_sym" >> $symbols_file
done
...
"x86_64-elf-objcopy" --redefine-syms="$sorted_file" "$target"
